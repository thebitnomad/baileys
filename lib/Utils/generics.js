...
const baileysVersion = [2, 3000, 1034485183]
const PLATFORM_MAP = {
    'aix': 'AIX',
    'darwin': 'Mac OS',
    'win32': 'Windows',
...
const fetchLatestBaileysVersion = async (options = {}) => {
    // Tier 1: Try wppconnect-team/wa-version GitHub Releases API
    // Fetch a stable version from ~2 weeks ago instead of latest
    try {
        const waVersionUrl = 'https://api.github.com/repos/wppconnect-team/wa-version/releases?per_page=100';
        const ghResult = await axios_1.default.get(waVersionUrl, {
            ...options,
            responseType: 'json',
            timeout: 8000,
            headers: {
        ...
        const releases = ghResult.data;
        const twoWeeksAgo = Date.now() - 14 * 24 * 60 * 60 * 1000;
        // Find the first release published at or before 2 weeks ago
        const stableRelease = releases.find(r => new Date(r.published_at).getTime() <= twoWeeksAgo);
        if (stableRelease) {
            const body = stableRelease.body || '';
            // Match specifically the WA version pattern: 2.3000.XXXXXXXXX (not the release tag like 1.5.3375)
            const versionMatch = body.match(/\b(2)\.(3\d{3})\.(\d{7,})/);
            if (versionMatch) {
                const version = [+versionMatch[1], +versionMatch[2], +versionMatch[3]];
                const publishedAt = stableRelease.published_at.split('T')[0];
                console.log(`🔄 WA Version (wa-version repo, stable from ${publishedAt}):`, version.join('.'));
                return { version, isLatest: true, source: 'wa-version-stable' };
            }
        }
    } catch (e) {
        // Tier 1 failed, continue to fallback
    }

    // Tier 2: Try scraping web.whatsapp.com/sw.js
    try {
        const swResult = await (0, exports.fetchLatestWaWebVersion)(options);
        if (swResult.isLatest && swResult.version) {
            console.log('🔄 WA Version (sw.js):', swResult.version.join('.'));
            return { version: swResult.version, isLatest: true, source: 'sw.js' };
        }
    } catch (e) {
        // Tier 2 failed, continue to fallback
    }

    // Tier 3: Hardcoded fallback
    console.log('⚠️ WA Version (fallback):', baileysVersion.join('.'));
    return {
        version: baileysVersion,
        isLatest: false,
        source: 'fallback'
