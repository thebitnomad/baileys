...
const fetchLatestBaileysVersion = async (options = {}) => {
    ...
    // Uses a 2-week delay for stability — avoids bleeding-edge versions
    try {
        ...
        while (!foundVersion && page <= 5) {
            const waVersionUrl = `https://api.github.com/repos/wppconnect-team/wa-version/releases?per_page=100&page=${page}`;
            ...
            for (const release of releases) {
                ...
                if (publishedAt <= twoWeeksAgo) {
                    ...
                    // Match WA version pattern: 2.3000.XXXXXXXXX (strips -alpha suffix automatically)
                    const versionMatch = body.match(/\b(2)\.(3\d{3})\.(\d{7,})/);
                    if (versionMatch) {
                        foundVersion = [+versionMatch[1], +versionMatch[2], +versionMatch[3]];
                        break;
                    }
                }
            }
        ...
        if (foundVersion) {
            console.log('🔄 WA Version (wa-version repo, 2-week delay):', foundVersion.join('.'));
            return { version: foundVersion, isLatest: true, source: 'wa-version' };
        }
    } catch (e) {
